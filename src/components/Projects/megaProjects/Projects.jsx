...
function Projects({ id, ProjectName, PorjectDescription, ProjectUrl, img , blobColor, source, handleProjectId}) {
  ...
  return (
    <div
      ref={ref}
      className={`techColor relative mb-[250px] h-[440px] rounded px-15 py-10 shadow-lg shadow-blue-950 over transition-all duration-700 ease-out select-auto ${
        delayPassed && inView
          ? "translate-y-0 opacity-100"
          : "translate-y-10 opacity-0"
      }`}
    >
      <div className={`flex ${source == "projectDetails" ? "gap-30" : "gap-20"} `}>
        {
